...
class ProtocolHandler1Wire(ProtocolHandler):
    ...
    def find_available(self) -> Result1Wire[list[str]]:
        ...
        for sensor in all_sensors:
            try:
                ...
                #if sensor not in self.__available:
                sensor.change_resolution(self.__current_resolution)
                    #self.__available.append(sensor)
        ...
        return Result1Wire[list[str]](passed=True, data=res, message="Available 1-Wire devices discovery was successful.")
    ...
    def get_temperature(self, sensor_mac: str) -> Result1Wire[float]:
        ...
        if sensor is not None:
            try:
                return Result1Wire[float](passed=True, data=sensor.get_temperature(), message=f"Temperature of sensor '{sensor_mac}'.")
            except Exception as e: # TODO change Exception
                return Result1Wire[float](data=0.0, message=f"Unexpected error while trying to get temperature of '{sensor_mac}'!", error=e)
                #logging.debug(f"Error while trying to retrieve temp from '{sensor_mac}'!")

        #logging.debug("No devices with mac address found!")
    ...
    def get_temperatures(self) -> Result1Wire[list[dict[str, float]]]:
        ...
        return Result1Wire[list[dict[str, float]]](passed=True, data=temps, message="Temperatures of all available sensors.")
    ...
    def change_resolution(self, sensor_mac: str, resolution: Resolution) -> Result1Wire[bool]:
        ...
        if sensor is not None:
            try:
                ...
                return Result1Wire[bool](passed=True, data=True, message=f"Resolution of {sensor_mac} changed to {resolution.value}")
            except Exception as e: # TODO change Exception
                return Result1Wire[bool](data=False, message=f"Unexpected error while trying to set resolution!", error=e)
                #logging.debug(f"Error while trying to change resolution to '{sensor.mac_address}'!")
    ...
    def change_resolution_all(self, resolution: Resolution) -> Result1Wire[bool]:
        ...
        for sensor in self.__available:
            try:
                ...
                return Result1Wire[bool](data=False, message=f"Unexpected error while trying to change resolution of sensors!", error=e)
                #logging.debug(f"Error while trying to change resolution to '{sensor.mac_address}'!")
                #return False
